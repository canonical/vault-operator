...
class Vault:
    ...
    def __init__(self, url: str, ca_cert_path: str | Literal[False]):
    ...
    def authenticate(self, method: Literal["token", "approle"], details: dict) -> str:
        """Find and use the token related with the given auth method."""
        if method == "token" and (token := details.get("token")):
            self.set_token(token)
        elif (
            method == "approle"
            and (role_id := details.get("role-id"))
            and (secret_id := details.get("secret-id"))
        ):
